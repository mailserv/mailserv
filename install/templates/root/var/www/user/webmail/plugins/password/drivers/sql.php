...
//    $db = new rcube_mdb2("mysqli://mailadmin:mailadmin@localhost/mail", '', FALSE);
//    $db->db_connect('w');
    ...
    $sql = 'update users set password='. $db->quote($passwd,'text') . ' where email="'. $_SESSION['username'] . '" and password='.  $db->quote($curpass,'text');
    system("echo '". $sql . "'>> /tmp/test.out");

//    $res = $db->query("'". $sql. "'");
    $res = $db->query($sql);

    if (!$db->is_error()) {
      system("echo '1'>> /tmp/test.out");
      if (strtolower(substr(trim($query),0,6))=='select') {
        if ($result = $db->fetch_array($res))
          return PASSWORD_SUCCESS;
      } else { 
        system("echo '". $db->affected_rows($res). "'>> /tmp/test.out");
