class Domain < ActiveRecord::Base
  ...
  def after_save
    system("/usr/local/bin/sudo /usr/local/bin/rake RAILS_ENV=production -f /var/mailserv/admin/Rakefile mailserver:configure:domains &")
  end
  ...
  def after_destroy
    %x{/usr/local/bin/sudo rm -rf /var/mailserv/mail/#{@oldname}}
  end
